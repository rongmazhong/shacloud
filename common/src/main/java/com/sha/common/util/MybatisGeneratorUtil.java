...
import static com.sha.common.util.StringUtil.lineToHump;
...
public class MybatisGeneratorUtil {
	...
	public static void generator(
		...
		try {
			...
			com.sha.common.util.VelocityUtil.generate(generatorConfig_vm, generatorConfig_xml, context);
			// 删除旧代码
		...
		for (int i = 0; i < tables.size(); i++) {
			String model = com.sha.common.util.StringUtil.lineToHump(ObjectUtils.toString(tables.get(i).get("table_name")));
			...
			if (!serviceFile.exists()) {
				...
				com.sha.common.util.VelocityUtil.generate(service_vm, service, context);
			...
			if (!serviceMockFile.exists()) {
				...
				context.put("model", model);
				context.put("ctime", ctime);
				com.sha.common.util.VelocityUtil.generate(serviceMock_vm, serviceMock, context);
			...
			if (!serviceImplFile.exists()) {
				...
				context.put("package_name", package_name);
				context.put("model", model);
				context.put("mapper", com.sha.common.util.StringUtil.toLowerCaseFirstOne(model));
				context.put("ctime", ctime);
				com.sha.common.util.VelocityUtil.generate(serviceImpl_vm, serviceImpl, context);
